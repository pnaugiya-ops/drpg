...
# --- 5. ADMIN PORTAL (Synced View) ---
elif st.session_state.role == "D":
    st.title("👩‍⚕️ Admin Master: Bhavya Labs")
    if st.button("🔄 Refresh Clinic Data"): st.rerun()

    # Read from Google Sheets
    df_global = conn.read(worksheet="Appointments", ttl=0)
    
    t1, t2, t3 = st.tabs(["📅 Appointments", "🩸 Patient Health Records", "📢 Social Updates"])
    
    with t1:
        st.subheader("Live Bookings")
        if not df_global.empty:
            st.dataframe(df_global[df_global['Type'] == "Appointment"], use_container_width=True)
        else:
            st.info("No active bookings.")

    with t2:
        st.subheader("Vitals & Lab Reports")
        if not df_global.empty:
            st.dataframe(df_global[df_global['Type'].isin(["Vitals", "Lab Report"])], use_container_width=True)
        else:
            st.info("No patient records uploaded yet.")

    with t3:
        st.info("Social Feed functionality is local to session.")
