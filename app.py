    ...
    if st.session_state.role == "D":
        ...
        if m == "Vitals":
            ...
            with st.form("v"):
                ...
                p = st.number_input("Pulse (bpm)", 40, 200, 72)
                bp = st.text_input("Blood Pressure", "120/80")
                if st.form_submit_button("Save"):
                    bmi = round(w/((h/100)**2), 1)
                    det = f"BMI:{bmi}, BP:{bp}, Pulse:{p}"
            ...
            if "Pregnant" in st.session_state.stat:
                st.write("**T-Dap:** 27-36 weeks | **Flu:** Anytime | **Tetanus:** Confirmation")
                
            else:
                st.write("**HPV Vaccine:** 3 doses (0, 1, 6 months) for Cervical Cancer prevention.")

        elif m == "Diet & Yoga":
            st.title("🧘 Nutrition & Yoga")
            if "Pregnant" in st.session_state.stat:
                ...
                with d2: 
                    st.write("**Diet:** Iron/Calcium focus. **Yoga:** Palm Tree Pose.")
                    

[Image of 2nd trimester pregnancy diet chart]

                with d3: 
                    st.write("**Diet:** High Fiber focus. **Yoga:** Supported Squats.")
            ...
            with st.form("u"):
                ...
                if st.form_submit_button("Send"):
                    new = pd.DataFrame([{"Name":st.session_state.name,"Type":"REPORT","Details":n,"Attachment":img_to_b64(f),"Timestamp":datetime.now().strftime("%Y-%m-%d %H:%M")}])
            ...
            with st.form("b"):
                ...
                tm = st.selectbox("Slot", ["10 AM", "11 AM", "12 PM", "5 PM", "6 PM", "7 PM"])
